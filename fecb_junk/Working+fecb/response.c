...
//--------FE
 ...
 //----------CB_DET
    if(i==0){ strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"CB_DET_CH2");}
     ...
     abuf[0] = stmpbuf[pt+5];
     pow_db=(double)convertdB((float)atof(abuf), 1); 
...
//------------------ VCC_M
         abuf[0]=stmpbuf[pt+2];      
         Monvoltbis[i*3] = ((float)atof(abuf)*10)/(float)256 -5;
...
//------------------ VEE_M
 ...
 //------------------ VREF_M
          abuf[0]=stmpbuf[pt+4];      
         Monvoltbis[2+i*3] = ((float)atof(abuf)*10)/(float)256 -5;
  ...
  //------------------ RF_POW_M
         if(i==0){strcpy(dev_Resp.resp_data.prmname[resp_msg_cntr],"RF_POW_M_CH2");}
          ...
          abuf[0] = stmpbuf[pt+5];
          sprintf(dev_Resp.resp_data.prmvalue[resp_msg_cntr],"%4.2f",atof(abuf));
...
#endif
                                        copyTmpBuf(stmpbuf,rbuff.Msg,(log2Len -tempSize));
