...
int cmd_validation(parseCMSCmd *Wrpr_Cmd, devResponse *MCM_Resp)
 {
   ...
   printf("\n==================> SUB SYSTEM ID %s",Wrpr_Cmd->cmdelem.subsysid);
   printf("\nDEFINED in FECB>H======> %s",SYSTEM_2);
...
//  printf("\nEntering Command_Process_FE");
  ...
  strcpy(MCM_Resp->resp_data.prmname[resp_msg_cntr],"time");
  ...
  { for (i = 0 ; i < TSET; i++ ) {
     ...
     { 
          //   printf("Inside the strcasecmp(Wrpr_Cmd->cmd_data.prmname[j], fevar[i]) Loop\n");
        switch(i)
        {
             case BAND_SELECT_CH1 :  fecbSet.freq_set[0] = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
                                     // printf("Value of Freq SET[0] %d\n",fecbSet.freq_set[j]);
                                     break;
             case BAND_SELECT_CH2 :  fecbSet.freq_set[1] = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]); //printf("Value of Freq SET[1] %d\n",fecbSet.freq_set[j]);
                                     break;
             case SOL_ATTEN_CH1   :  fecbSet.solar_attn[0] = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);//printf("Value of Freq Solar Attan[0] %d\n",fecbSet.solar_attn[j]);
                                     break;
             case SOL_ATTEN_CH2   :  fecbSet.solar_attn[1] = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);//printf("Value of Freq Solar Attan[1] %d\n",fecbSet.solar_attn[j]);
                                     break;
             case RF_SWAP         :  fecbSet.polSwap = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);//printf("Value of POl SWAP %d\n",fecbSet.polSwap);
                                     break;
             case FE_NGCAL        :  fecbSet.noiseCal = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);//printf("Value of Noise Cal %d\n",fecbSet.noiseCal);
                                     break;
             ...
             case RFCM_SW         :  fecbSet.rfcmSW = atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);//printf("Value of RFCM SW %d\n",fecbSet.rfcmSW);
                                     break;
