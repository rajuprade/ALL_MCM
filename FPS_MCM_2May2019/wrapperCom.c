...
ssize_t writen(int fd, void *vptr, ssize_t nbytes)
{
  ...
  write_ptr = (unsigned char *) malloc(parseCMSCmdSize); // ADDED BY RU 
  if (write_ptr == NULL) 
      {
        fprintf(stderr, "Unable to allocate enough memory for Write Ptr!\n");
        return -1;
      }
  bzero(write_ptr,parseCMSCmdSize);
  write_ptr = ( unsigned char  *) vptr;
  while ( n < nbytes) {
      //printf("#################In Written Function before IF loop: %d\n",fd);
     if ((nbytes - n) > MaxProtBuf) l = MaxProtBuf; else l = nbytes - n;
     //  printf("#################In Written Function before Socket block : %d\n",fd);
        so_block(fd,0); 
        nwritten = write(fd,write_ptr+n,l);
    // nwritten = send(fd,write_ptr+n,l, 0);
 //     ioctl (fd, I_FLUSH, FLUSHW); /* Flush the data to Socket */
      //printf("#################In Written Function before Socket unblock loop: %d\n",fd);
       so_block(fd,1);
      //  printf("#################In Written Function After Socket unblock: %d\n",fd);
...
ssize_t readn(int fd, void *vptr, ssize_t nbytes)
{
  ...
  read_ptr = (unsigned char *) malloc(devResponseSize); // ADDED BY RU 
  if (read_ptr == NULL) 
      {
        fprintf(stderr, "Unable to allocate enough memory for read Ptr!\n");
        return -1;
      }
   bzero(read_ptr,devResponseSize);
