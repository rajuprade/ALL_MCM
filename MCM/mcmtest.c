...
//#define DEVICE_ID "FT1KF7AP"
#define DEVICE_ID "FT1KFDBG"

/*** Block to Handle Terminate Signal Date 24/07/2020 ****/

#include<signal.h> // Added on 24/07/2020
...
void sig_handler(int signo)
{
        if (signo == SIGTERM)
        {
          printf("Received KILL SIGNAL, CLOSING HANDLE AND RTS LINE \n");
          ftStatus = FT_ClrRts(ftHandle);
          FT_Close(ftHandle);
          exit(-1);
        }
    else printf("#### PROGRAM NOT TERMINATED CLEAN ##### \n");
}

/*************** Block to Handle Signal Stop Here ****/ 
...
{ 
    ...
    resp_fecbfd = open(respfecb,O_RDWR|O_NONBLOCK); 
      ...
      iflo_fd = open(iflofifo,O_RDONLY|O_NONBLOCK); 
        ...
        if (signal(SIGTERM, sig_handler) == SIG_ERR) // Signal handler to properly close the program
        printf("\ncan't catch SIGTERM\n");
	...
	usleep(250); //sleep added to pause loop DBB
    } 
    return 0; 
} 
