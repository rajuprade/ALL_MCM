...
int  mcmPow(int mcmSW)
{
    ...
    {
       case 0 : // MCM OFF
// First ON & Then OFF !! following rituals -- $JPK Oct 11, 2019 $
...
int fecbCtrlCmd(parseCMSCmd *Wrpr_Cmd)
{
   ...
   switch (fecbSet.cmdCode)
   {
        ...
        // MCM4 Enabled Functionality
        case WALSHPATERN:  
        case WALSHFREQ: 
        case NOISEFREQ:
        case SETWALSHGRP: 
        case SETWALSH: 
        case RFNGSET:    // ?? Noise ON OFF
        case SETRFNOISE: //??    // ?? Noise ON OFF
        case RFCM_SW: 
                      mcm4setting();
                      mcmPow(1); sleep(1);
#ifdef MCM5OFF
        ...
        //-------------< Old RF-CB Commands for non-broadband>-----------// subband not available.
        case SETRFSYS :  
       ...
       case FESDOMON  :
             ...
             if ( fecbSet.cmdCode == RAWMON ) 
             {
                  ...
                  mcmPow(1);sleep(1);
                   ...
                   bzero(&femonPkt, devResponseSize);
                   dev_Resp.resp_data.numpkt = resp_msg_cntr;
                   memcpy(&femonPkt, &dev_Resp, devResponseSize); 
                   fecbSet.monEnable = 1; // will store Monitoring Data
                   mcmPow(0);
             } 
#else
             if(exeCmd.McmPow == 0 )
       ...
       // RFNGCAL and RF Term (Hi, EHi, Med, Low, Term in FE BOX to Upgraded Box
          case SETURFSYS :       
          case RFNGCAL:   
          case RFTERM :
