...
int command_process_IFLO(parseCMSCmd *Wrpr_Cmd, devResponse *MCM_Resp)
{
  ...
  char *fevar[] =  { "ilo_ch1","ilo_ch2","ifbw_ch1" ,"ifbw_ch2" ,"pre_attn_ch1","pre_attn_ch2","post_gain_ch1" ,"post_gain_ch2" ,"ifalc_ch1" ,
                    "ifalc_ch2","ofcatten_ch1","ofcatten_ch2","fe_walsh_sw" ,"fe_walsh_grp" ,"fe_ngcycle" ,"rfcm_sw", "noisefreq","walshpattern","walshseq","walshfreq"};
  typedef enum { ILO_CH1,ILO_CH2,IFBW_CH1,IFBW_CH2, PRE_ATTN_CH1,PRE_ATTN_CH2,POST_GAIN_CH1,POST_GAIN_CH2,IFALC_CH1,IFALC_CH2,OFCATTEN_CH1,OFCATTEN_CH2,FE_WALSH_SW, FE_WALSH_GRP, FE_NG_CYCLE, RFCM_SW, NOISEFREQ,WALSHPATTERN,WALSHSEQ,WALSHFREQ,TSET};
        ...
        switch(i)
        {
            ...
            case  OFCATTEN_CH2   : ofc_ch2 = (int) atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
             ...
             case WALSHPATTERN   :  fecbSet.walshPat =(int)atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
                                    printf("######### WALSH PATTERN in IFLO_ETH is %d\n",fecbSet.walshPat);
                                    if( fecbSet.walshPat > 7 || fecbSet.walshPat  < 0 )
                                     fecbSet.walshPat = 0;
                                     break;

             case WALSHSEQ  :  fecbSet.walshseq =(int)atoi(Wrpr_Cmd->cmd_data.prmvalue[j]);
                                    printf("######### WALSH SEQ in IFLO_ETH is %d\n",fecbSet.walshPat);                                    
                                      break;
